    ...
    with open(dst_path, 'w', newline='') as file_out:
        ...
        for path in list2:
            ...
            if math.isnan(epy):
                epy = 0
            try:
                time_str = data.get('time', "").replace("Z", " ").replace("T", " ").replace(".000Z","").replace(".000 ", "")
                time_obj = datetime.datetime.strptime(time_str, '%Y-%m-%d %H:%M:%S')
            except ValueError:
                time_obj = datetime.datetime(int(year), int(month), int(day), int(hour), int(minutes), int(seconds))

            csv_out.writerow([year, month, day, hour, minutes, seconds, lat, lon, alt, speed, heading, altHAE, eps, ept, epv, epx, epy, time_obj])
            row += 1
